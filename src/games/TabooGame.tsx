...
import { Plus, Minus, Play, RotateCcw, AlertTriangle, Users, Ban } from 'lucide-react'
import { useI18n } from '../i18n'
import { useGameText } from '../hooks/useGameText'
import { useTimer } from '../hooks/useTimer'
import { tabooCards, tabooCardsEn, type TabooCard } from '../data/taboo-words'
...
export default function TabooGame() {
  ...
  const [cardsUsed, setCardsUsed] = useState<Set<string>>(new Set())
  const [round, setRound] = useState(1)

  const cards = useMemo(() => {
    const source = lang === 'ru' ? tabooCards : tabooCardsEn
    return [...source].sort(() => Math.random() - 0.5)
  }, [lang])

  const timer = useTimer(timerSeconds)

  const getNextCard = (): TabooCard => {
    const available = cards.filter(c => !cardsUsed.has(c.word))
    if (available.length === 0) {
      setCardsUsed(new Set())
      return cards[Math.floor(Math.random() * cards.length)]
    }
    const card = available[Math.floor(Math.random() * available.length)]
    setCardsUsed(prev => new Set(prev).add(card.word))
    return card
  }
  ...
  if (timer.isFinished && phase === 'playing') {
    const newTeams = teams.map((team, i) =>
      i === currentTeam ? { ...team, score: Math.max(0, team.score + turnScore) } : team
    )
    setTeams(newTeams)
  ...
  const addTeam = () => {
    setTeams(prev => [...prev, { name: `${L('Команда', 'Team')} ${prev.length + 1}`, score: 0 }])
  }
  ...
  if (phase === 'setup') {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold">🚫 {L('Табу', 'Taboo')}</h2>
        ...
        <div className="card p-4 text-sm text-text-secondary space-y-1">
          ...
          <p>2. {L('Объясняющий видит слово и запрещённые слова', 'The explainer sees a word and forbidden words')}</p>
          <p>3. {L('Объясните слово, НЕ используя запрещённые', 'Explain the word WITHOUT using forbidden words')}</p>
          <p>4. {L('Угадали: +1, нарушение табу: −1, пропуск: 0', 'Correct: +1, taboo violation: −1, skip: 0')}</p>
        </div>
        ...
        <div className="card p-4 space-y-4">
          <div>
            ...
            <input type="range" min={5} max={30} step={5} value={targetScore}
    ...
    return (
      <div className="space-y-6 text-center">
        ...
        <div className="card p-4 text-sm text-text-muted space-y-1">
          <p>{G('reminder')}</p>
          <p>&bull; {L('Нельзя произносить запрещённые слова', 'Do not say any forbidden words')}</p>
          <p>&bull; {L('Нельзя использовать жесты и звуки', 'No gestures or sounds')}</p>
          <p>&bull; {L('Соперники следят за нарушениями', 'Opponents watch for violations')}</p>
        </div>
  ...
  // ═══════════════════════════════════════════
  // PLAYING — card + timer + correct/taboo/skip
  // ═══════════════════════════════════════════
  if (phase === 'playing') {
      ...
      <div className="space-y-5 text-center">
        ...
        {currentCard && (
          <div className="card p-6">
            <p className="text-3xl sm:text-4xl font-bold leading-tight mb-4">{currentCard.word}</p>
            <div className="border-t border-border pt-4 space-y-2">
              <p className="text-xs text-red-400 uppercase tracking-wider font-medium flex items-center justify-center gap-1">
                <Ban size={12} /> {L('Запрещённые слова', 'Forbidden words')}
              </p>
              {currentCard.taboo.map((fw, i) => (
                <p key={i} className="text-red-400/80 text-lg font-medium">{fw}</p>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2">
          <button onClick={handleTaboo}
            className="py-5 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-semibold
              active:scale-95 transition-transform touch-manipulation">
            <Minus size={24} className="mx-auto mb-1" />
            {L('Табу!', 'Taboo!')}
          </button>
          <button onClick={handleSkip}
            className="py-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-sm font-semibold
              active:scale-95 transition-transform touch-manipulation">
            <span className="text-2xl block mb-1">→</span>
            {t.game.skip}
          </button>
          <button onClick={handleCorrect}
            className="py-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-semibold
              active:scale-95 transition-transform touch-manipulation">
